...
class test_prm_v0(PRM):
    def __init__(
        ...
        self.bad_token = '-'
        self.step_tag = 'ки'
        self.model_id = "mtzig/v0_mistral_lora_last_n_train_3"
        ...
        ###
        if self.quantization_config:
            self.model = AutoModelForCausalLM.from_pretrained('mtzig/v0_mistral_lora_last_n_train_3',
                                                       quantization_config=quantization_config)
        else:
            self.model = AutoModelForCausalLM.from_pretrained('mtzig/v0_mistral_lora_last_n_train_3')
        ###

        # check_model_precision(self.model)
        # input()
        ###
