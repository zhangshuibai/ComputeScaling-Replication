...
class test_prm_dual(PRM):
    def __init__(
        ...
        self.model_id = "tpzeng/prm800k_llama_joint_checkpoint1000"

        self.tokenizer = get_tokenizer(self.model_id)

        # Candidate tokens updated
    ...
    def __call_single(self, single_beam: str) -> list[float]:
        ...
        # If no separators found, treat entire input as a single step
        if not sep_positions:
            # sep_positions = [total_length]
            raise ValueError
        ...
        # Iterate over each step starting from 第二个分隔符（跳过Question部分）
        for i in range(1, len(step_boundaries)-1):
            ...
            # print(self.tokenizer.decode([input_ids[last_token_index]]))
    ...
    def __call__(self, steps: list[str]) -> list[StepScore]:
        ...
        for beam in steps:
            # step_scores = self.__call_single(beam)
            # # Split steps based on "\n\n" for associating scores
            # decoded_text = self.tokenizer.decode(self.tokenizer.encode(beam))
            # steps_split = decoded_text.split(" \n\n")[1:]  # Skip the Question part
            # print(steps_split)
            # print(step_scores)
            # input("+++")
            # for step, score in zip(steps_split, step_scores):
            #     result.append(StepScore(step=step, score=score))
